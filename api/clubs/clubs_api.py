...
async def get_club_details_by_id(user: Annotated[UserInternal, Depends(RoleBasedAccessControl({UserRoles.USER}))], club_id: str) -> ClubExternal | None:
    """
    :param user:
...
async def book_riding_lesson_service(club_review: ClubReview, user: Annotated[UserInternal, Depends(RoleBasedAccessControl({UserRoles.USER}))]) -> dict:
    log.info(f"adding user: {user}'s review for club {club_review.club_id}")
