...
- The user signs up using the `users/signup` route
- The user use this route to enlist a horse for renting
- This route may also be used by a `club`
- The differences between a `user` and a `club` are the following:
    1. firstly, the`user_role` of an `user` is set to `USER`, for a club it's `CLUB`.
    2. Secondly, a `club` has a `club` entity associated with it. The
...
1. A new document in the `horses` collection will be created. The schema of
   the document will be similar to the following:
    ...
    ```
2. Obtain the `horse_id` that was generated in the previous step and create a new
   document in the `horse_renting_service` collection. The schema of the document will
   be similar to the following:
...
On completion of all the operations, return the `horse_renting_service_id`.

```json
{
  "horse_renting_service_id": "horse_renting_service_id"
}
```

### 2. `user/horses/{horse_renting_service_id}/upload-rent-images`

After enlisting a horse for rent using the `user/horses/enlist-horse-for-rent` route
the user will use this route to upload images of the horse.
...
1. The user will provide a list of images to be uploaded
   and associated with the `horse_renting_service`.
...
1. The `horse_renting_service_id` provided as a path parameter must have been
   created by the requesting user.
...
1. The images provided by the user will be saved using the `save_image()` function of
   the image handling mechanism.
2. The `image_id`s returned by the image handling mechanism would be saved in the same
   `horses` collection in the document associated with the `horse_renting_service_id`.
   The key for saving the images would be `images`.
    ```json
    {
      "_id": ObjectId("12345"),
...
- The `user` uses this route for two purposes.
    1. to get all the horses that are listed for rent by others
    2. to get all the horses that were listed by him
- We use a query parameter to figure out whether to send the horses listed by
  others, or the horses listed by him.
...
1. Based on the query parameter it would be decided whether to return the listings
   added by the `user` or to return listings added by others.
2. If the query parameter `own_listing=true`, return the listings of the user.
   Otherwise, return the listing of others.
3. While returning the listing of others make sure to exclude items that were
   created by the user himself.
5. Query the `horse_renting_service` collection. Use `mongodb aggregate pipeline`
   and `lookup` to join the collection with the `horses` collection. Return the data
   to the user.
...
[
  {
    ...
    "price": "horse_renting_service.price",
    "seller_information": {
      "name": "user.name / club.name",
      "email_address": "fXXXXXXXXX@gmail.com",
      "phone_no": "+91XXXXXXXXXX",
...
#### Request Validations
1. The `provider.provider_id` of the `horse_renting_service` must match
   with `user._id` or `club._id` depending on the type of the `provider`.
...
#### The Flow:
1. Update the fields of the relevent documents of the corresponding collections
   i.e. `horse_renting_service` and `horses`. 
...
- User accesses the rent listing using the `/user/horses/get-horses-for-rent` route.
- User creates an enquiry for a listing. The enquiry will be visible to admins.
...
```json
{
    "horse_selling_service_id": "the id",
    "message": "the message of the user",
    "date": "which date does the user want to rent the horse from",
    "duration": "how many days does the user want to rent the horse for" 
}
```
...
1. A new document in the `horse_renting_enquiry` collection will be created. The schema of
   the document will be similar to the following:
    ...
    ```json
    {
        "_id": ObjectId("12345"),
        "user_id": "the id of the user",
        "horse_renting_service_id": "the horse_selling_renting_id",
        "message": "the message of the user",
        "date": "which date does the user want to rent the horse from",
        "duration": "how many days does the user want to rent the horse for"
    }
...
```json
{
  "horse_renting_enquiry_id": "the id of the newly created record"
}
```
...
#### The Process
1. `users` will use this route to update the `rent-enquiry` it has already made using the
   `user/horses/enquire-for-a-horse-rent` route.

#### Path Parameters
1. `horse_renting_enquiry_id`: The `_id` of the `horse_selling_enquiry` that the user wants to update.
...
#### Request Validations
1. The user must be the creator of the enquiry. That is, the `horse_renting_enquiry.user_id` must
   match with the `user._id`.
...
#### The Flow:
1. Update the `message` of the corresponding `horse_renting_enquiry` document.
...
- Users will use this route to get the `renting-enquiries` made by himself.
- Admins will use this route to get the `renting-enquiries` made by all other users.
...
[
    {
        "horse_renting_enquiry_id": "horse_renting_enquiry._id",
        "horse_renting_service_id": "the horse_renting_service_id",
        "user_id": "horse_renting_enquiry.user_id",
        "message": "horse_renting_enquiry.message",
        "date": "horse_renting_enquiry.date",
        "duration": "horse_renting_enquiry.duration"
    }
]
```
