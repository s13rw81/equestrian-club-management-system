...
def save_trainer_certifications_bulk(
    new_trainer_certifications: list[TrainerCertificationInternal], session=None
) -> list[TrainerCertificationInternal]:
    log.info(
        f"inside save_many_trainer_certifications(new_trainer_certifications={new_trainer_certifications})"
    )
    ...
    new_trainer_certifications_dumped = [
        new_trainer_certification.model_dump()
        for new_trainer_certification in new_trainer_certifications
    ]

    result = trainer_certification_collection.insert_many(
        new_trainer_certifications_dumped, session=session
    )

    if not result.acknowledged:
        log.info(
            "new trainer certifications can't be inserted in the database, raising exception..."
        )
        raise HTTPException(
...
def update_trainer_certifications_bulk(
    ...
    log.info(
        f"inside update_trainer_certifications_bulk("
    ...
    for update_trainer_certification_dto in update_trainer_certification_dtos:
        ...
        update_trainer_certification_dict = update_trainer_certification_dto.model_dump(
    ...
    result = trainer_certification_collection.bulk_write(updates, session=session)
    ...
    if not result.modified_count:
        log.info(
            "could not update trainer certifications due to unknown reasons, raising exception"
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
