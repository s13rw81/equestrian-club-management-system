from data.db import get_horses_collection
from pymongo.collection import Collection
from api.horses.models.create_horse import HorseInDB
from models.horse.horse_update_internal import InternalUpdateHorse
from api.horses.models.update_horse import HorseUpdate
from pymongo import ReturnDocument
collection: Collection = get_horses_collection()


def create_horse(horse: InternalUpdateHorse):
    # Insert the horse document into the collection
    result = collection.insert_one(horse.dict(by_alias=True))

    # Retrieve the inserted document's ID
    horse_id = result.inserted_id
    return str(horse_id)


def update_horse(horse_id: str, horse_data: InternalUpdateHorse):
    updated_horse = collection.find_one_and_update(
        {"_id": horse_id},
        {"$set": horse_data},
        return_document=ReturnDocument.AFTER
    )

    if updated_horse:
        return HorseUpdate(**updated_horse)
    return None


def delete_horse(horse_id: str):
    delete_result = collection.delete_one({"_id": horse_id})
    return delete_result.deleted_count == 1
